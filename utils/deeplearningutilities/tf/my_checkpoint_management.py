...
#         print(self._all_steps_checkpoints)
    ...
    def save(self, step, force=False):
        """Always writes a new checkpoint and cleans up old checkpoints.

        step: The step value of the checkpoint to write. The step will be part of the filename.

        force: If True this function will overwrite existing checkpoints.
        """
        current_step = int(step)

        prefix = '{0}-{1}'.format(self._checkpoint_prefix, current_step)
        if (current_step, prefix) in self._all_steps_checkpoints:
            if force:
                ...
                print('found existing checkpoint',
                      prefix,
                      'while saving! Not overwriting existing checkpoint.',
                      flush=True)
                return
